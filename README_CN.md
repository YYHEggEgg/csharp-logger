...
- 修复了在现代终端中手动粘贴超长命令时输入越来越卡顿、输入区反复被清空、提示符被日志覆盖或回车长时间无法生效而看起来像命令不完整的问题。
- 修复了日志文件初始化失败后再次初始化时控制台或 Logger 状态残留、导致重试失败的问题；修复了标准输入/输出被重定向时仍进入交互模式、控制台后台异常后输入/输出停止，以及程序退出时最后几条日志没有写完的问题。
- 修复了连续输出日志时输入区长时间没有响应、提示符消失或错位、日志显示顺序异常，以及旧的取消操作清空新输入的问题。
- 修复了取消 `ReadLineAsync` 后吞掉下一行、无人读取时后台提前读入、在行中移动光标后按 Enter 提交错误内容，以及粘贴的后一行输入丢失的问题。
- 修复了输入 Emoji、中文、全角字符或组合字符时，移动、删除、粘贴、调用历史记录或在终端边缘换行会拆开字符、错置光标或留下残影的问题；粘贴控制字符/ANSI 序列不再破坏终端显示，AltGr 与 ISO 102 键也可以正常输入。
- 修复了自动补全处理器返回非法范围时误删无关输入、输入区卡死或后续按键不再响应的问题；处理器抛出异常后补全列表也会恢复正常。
- 修复了缩小或改变终端窗口后输入冻结、持续刷出异常、持久区域或进度区域残留旧行的问题；部分 Unix 终端不再出现日志必须等待下一次按键才刷新的情况，非法持久区域间隔也不会令显示停止。
- 修复了进度条百分比和边界显示错误、非法颜色标签影响后续输出、零宽/组合字符导致行数统计错误、自动补全示例插入位置错误，以及 `LogTextWriter` 在换行和释放时产生重复或空日志的问题。`LogTextWriter.Flush()`/`FlushAsync()` 会提交未换行内容，退出时会尽力刷新未自动刷新的文件，释放后继续写入会明确失败。
- 修复了同时写入普通格式和管道格式文件时日志头部格式错误、一个文件写入变慢时连带阻塞其他文件，以及极少数并发构造失败时误关闭其他 Logger 正在使用的文件并静默丢失后续日志的问题。

#### 行为变更

- `BaseLogger.RefreshLogMilliseconds` 与 `ConsoleWrapper.RefreshTicks` 必须为正数。持久区域渲染器短于 15 ms 的回调间隔（包括非正数）会按 15 ms 处理。
- 并发的 `ReadLine`/`ReadLineAsync` 调用将被串行处理。标准输入或输出被重定向时，`ConsoleWrapper` 使用普通流输入/输出，不启用交互式编辑和持久区域。
- `ProgressBarBlocks` 必须大于零；进度值会被限制在 0 到 1 之间。
- `LogTextWriter.Flush()` 与 `FlushAsync()` 会将当前缓冲区作为一条日志提交；对象释放后的写入会抛出 `ObjectDisposedException`。

### v6.0.1

- 去除了退出清理工作消耗 1s 的超时限制，以避免工具类程序写入大量数据时可能发生的数据丢失。
- 修复了使用 `ConsoleWrapper` 时，在某些情况下大幅更改控制台高度（例如 VSCode 集成终端的展开/缩小）会导致控制套停止响应并产生大量报错的情况。

#### 已知问题

- 在**有调试器附加的情况下**，如果 `ConsoleWrapper.ShutDownRequest` 事件注册的处理程序没有执行手动调用 `Environment.Exit` 终止程序，或是将 `e.Cancel` 设为 `true`，则程序会在事件处理程序返回后保持无响应状态，而不是像预期的那样终止程序。  
  由于没有调试器附加时该问题不会出现，因此怀疑是调试器对 Ctrl+C 信号的处理与预期不符所致，对此问题没有特别的处理计划。与 [最佳实践](#最佳实践) 中提到的建议一致，建议在事件处理程序中手动调用 `Environment.Exit` 以确保程序能够正确终止，这样也可以保证 Logger 清理程序的运行。
